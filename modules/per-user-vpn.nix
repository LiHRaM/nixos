...
# This module defines the `saga.services.per-user-vpn` options for setting up an OpenVPN
...
with lib.types;
let
  cfg = config.saga.services.per-user-vpn;
in
{
  options.saga.services.per-user-vpn = {
      ...
      type = attrsOf (
        submodule {
          options = {
            ...
            tls_auth = mkOption {
              description = "Path to file containing certificate for tls";
              type = path;
            };
